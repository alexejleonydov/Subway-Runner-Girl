...
Shader "Unlit/Texture NGUI"
{
  Properties
  {
    _MainTex ("Base (RGB), Alpha (A)", 2D) = "black" {}
    _ClipUp ("ClipUpLevel", float) = 1
    _ClipDown ("ClipDownLevel", float) = 1
  }
  SubShader
  {
    Tags
    ...
    Pass // ind: 1, name: 
    {
      ...
      OUT_Data_Vert vert(appdata_t in_v)
      {
          ...
          out_v.xlv_TEXCOORD1 = tmpvar_1.xy;
          return out_v;
      }
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          OUT_Data_Frag out_f;
          float4 col_1;
          if((in_f.xlv_TEXCOORD1.y>=_ClipUp))
          {
              discard;
          }
          if((in_f.xlv_TEXCOORD1.y<=_ClipDown))
          {
              discard;
          }
          float4 tmpvar_2;
