...
public class UIInput : MonoBehaviour
{
	...
	public KeyhelmType keyhelmType;
	...
	protected static TouchScreenKeyboard mKeyhelm;

	private static bool mWaitForKeyhelm;
	...
	public string value
	{
		...
		set
		{
			...
			if (isSelected && mKeyhelm != null && mCached != value)
			{
				mKeyhelm.text = value;
				mCached = value;
			}
	...
	public int cursorPosition
	{
		get
		{
			if (mKeyhelm != null && !inputShouldBeHidden)
			{
				return value.Length;
			}
		...
		set
		{
			if (isSelected && (mKeyhelm == null || inputShouldBeHidden))
			{
				mSelectionEnd = value;
				UpdateLabel();
			}
	...
	protected void OnDeselectEvent()
	{
		...
		if (label != null && NGUITools.GetActive(this))
		{
			...
			if (mKeyhelm != null)
			{
				mWaitForKeyhelm = false;
				mKeyhelm.active = false;
				mKeyhelm = null;
			}
	...
	protected virtual void Update()
	{
		...
		if (mWaitForKeyhelm)
		{
			if (mKeyhelm != null && !mKeyhelm.active)
			{
				return;
			}
			mWaitForKeyhelm = false;
		...
		if (mSelectMe != -1 && mSelectMe != Time.frameCount)
		{
			...
			if (platform == RuntimePlatform.IPhonePlayer || platform == RuntimePlatform.Android || platform == RuntimePlatform.WP8Player || platform == RuntimePlatform.BlackBerryPlayer || platform == RuntimePlatform.MetroPlayerARM || platform == RuntimePlatform.MetroPlayerX64 || platform == RuntimePlatform.MetroPlayerX86)
			{
				...
				if (inputShouldBeHidden)
				{
					...
					keyboardType = (TouchScreenKeyboardType)keyhelmType;
					text = "|";
				}
				...
				mWaitForKeyhelm = true;
				mKeyhelm = ((inputType != InputType.Password) ? TouchScreenKeyboard.Open(text, keyboardType, !inputShouldBeHidden && inputType == InputType.AutoCorrect, label.multiLine && !hideInput, false, false, defaultText) : TouchScreenKeyboard.Open(text, keyboardType, false, false, true));
			}
			else
			{
		...
		if (mKeyhelm != null)
		{
			string text2 = ((!mKeyhelm.done && mKeyhelm.active) ? mKeyhelm.text : mCached);
			if (inputShouldBeHidden)
			{
				if (text2 != "|")
				{
			...
			{
				mCached = text2;
				if (!mKeyhelm.done && mKeyhelm.active)
				{
					value = text2;
				}
			}
		...
		if (!mCam.useKeyhelm && UICamera.GetKeyUp(KeyCode.Tab))
		{
			OnKey(KeyCode.Tab);
		}
	}
	...
	public void UpdateLabel()
	{
		...
		label.text = text2;
		if (flag && (mKeyhelm == null || inputShouldBeHidden))
		{
