...
public class ObjExporter
{
	...
	public static string MeshToString(MeshFilter mf, float scale)
	{
		...
		{
			Vector2 vector2 = Vector2.Scale(mesh.uv[k], textureScale) + textureOffset;
			if (dictionary.ContainsKey(k))
			{
				stringBuilder.Append(string.Format("vt {0} {1}\n", mesh.uv[k].x, mesh.uv[k].y));
			}
			else
			{
				stringBuilder.Append(string.Format("vt {0} {1}\n", vector2.x, vector2.y));
		...
		for (int l = 0; l < mesh.subMeshCount; l++)
		{
			...
			int[] triangles2 = mesh.GetTriangles(l);
			for (int m = 0; m < triangles2.Length; m += 3)
			{
				stringBuilder.Append(string.Format("f {0}/{0} {1}/{1} {2}/{2}\n", triangles2[m] + 1, triangles2[m + 1] + 1, triangles2[m + 2] + 1));
			}
		}
